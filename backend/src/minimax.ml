  ...
  for x = 0 to 15 - 1 do
    for y = 0 to 15 - 1 do
      if CoordMap.find_exn update (x, y) = 0 then ()
        ...
        let current_player = CoordMap.find_exn update (x, y) in
        let count_horizontal = count_in_a_row update x y 1 0 current_player in
        let count_vertical = count_in_a_row update x y 0 1 current_player in
        let count_diag_left = count_in_a_row update x y 1 1 current_player in
        let count_diag_right =
          count_in_a_row update x y 1 (-1) current_player
        in
...
let rec minimax_ab (state : board_state) (depth : int) (is_max_branch : bool)
  ...
  if depth = 0 then let score = heuristic_func state.board in 
  if score > 10 then printf "score of board: %d\n%s" score (Game.print_board state.board);
  score
          ...
          | Ok new_pieces ->
              let value =
                minimax_ab
                  ...
                  (depth - 1) true alpha beta other_player
              in
          ...
          | Ok new_pieces ->
              let value =
                minimax_ab
                  ...
                  (depth - 1) false alpha beta cur_player
              in
