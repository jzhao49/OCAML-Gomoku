...
let rec minimax_ab (state : board_state) (depth : int) (is_max_branch : bool)
  ...
  if depth = 0 then printf "score of board: %d\n" (heuristic_func state.board);
  if depth = 0 then heuristic_func (state.board)
    ...
    try
      List.iter
        ~f:(fun coord ->
          ...
          let new_board =
            insert_piece_no_error state.board coord other_player
          in
          let new_state = { board = new_board; player = 2 } in
          (* printf "reached line 189\n"; *)
          let value =
            minimax_ab new_state (depth - 1) (not is_max_branch) alpha beta
              other_player
          in
...
let find_best_move (state : board_state) (depth : int) (cur_player : int) :
  ...
  List.iter
    ~f:(fun avail_pos ->
      let new_state =
        {
          board = insert_piece_no_error state.board avail_pos cur_player;
          player = cur_player;
        }
      in
      let value =
        minimax_ab new_state (depth - 1) true (ref 0) (ref 0) cur_player
      in
